...
//! - MCP detection reads project-level config files
...
pub async fn get_context_health(
    ...
    // Conversation tokens are estimated as remaining usage minus persistent context
    // In practice this would come from live session data; we estimate a baseline
    let conversation_tokens = estimate_conversation_tokens();
...
fn estimate_mcp_tokens(project_path: &std::path::Path) -> u32 {
    ...
    // Each MCP server tool definition adds overhead to the context
    // Estimate ~500 tokens per server for tool schemas
    let mcp_json = project_path.join(".mcp.json");
    if mcp_json.exists() {
        if let Ok(content) = std::fs::read_to_string(&mcp_json) {
            let server_count = content.matches("\"command\"").count()
                + content.matches("\"url\"").count();
            tokens += (server_count as u32) * 500;
        }
    }
...
/// Estimate baseline conversation tokens.
/// In a real implementation this would come from live session data.
/// We return a baseline estimate for static analysis.
fn estimate_conversation_tokens() -> u32 {
    // Baseline: assume ~5000 tokens of conversation overhead
    5000
}

/// Parse MCP config JSON to extract server entries.
fn parse_mcp_config(content: &str, servers: &mut Vec<McpServerStatus>) {
    // Simple heuristic parsing — look for "mcpServers" object keys
    // A proper implementation would use serde_json, but we keep it lightweight
    // by scanning for patterns in the JSON content
...
mod tests {
    ...
    fn test_estimate_conversation_tokens() {
        assert_eq!(estimate_conversation_tokens(), 5000);
    }
